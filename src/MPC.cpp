...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    //**************************Establish Constraints - ie - the "Model" part of MPC ************************

    //Fill in the fg vector. 
    //It's shifted by 1 from vars since the cost is stored at fg[0]

    //The first value in each "chapter" of values is just the current state, stolen 
    //right from the vars vector. 
    ...
    //From there we must cycle through each time step and establish the relationship 
    //between each state (t+1) and the state before (t) based on the equations of motion
    for (int t = 1; t < N; t++) {
      
      // Essentially unpacking variables for readability - sourcing from known vars vector
